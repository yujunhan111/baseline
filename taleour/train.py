...
def main(batch_size, num_epochs, learning_rate, gradient_accumulation_steps, sample_points, patience,lr_weight):
    ...
    # 定义训练数据文件路径
    train_data_paths = [rf"C:\Users\yujun\PycharmProjects\ehr_迁移\rebuttal用\data/patient_records_batch_{i}.pkl" for i in range(1, 15)]
    # 加载验证数据
    val_data_paths = [
        r"C:\Users\yujun\PycharmProjects\ehr_迁移\rebuttal用\data/patient_records_batch_15.pkl",
        r"C:\Users\yujun\PycharmProjects\ehr_迁移\rebuttal用\data/patient_records_batch_16.pkl"
    ]
    # 加载人口统计数据
    directory_path = r"C:\Users\yujun\PycharmProjects\ehr_迁移\rebuttal用\data/patients_dict.csv"
    patient = pd.read_csv(directory_path)
    mappings = create_all_mappings(patient)
    # 加载词嵌入数据
    directory_path = r"C:\Users\yujun\PycharmProjects\ehr_迁移\rebuttal用\data\code_dict.parquet"
