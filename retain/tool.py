...
def get_history_codes(visits, current_time, index_set, days_before=0):
    """
    收集疾病代码出现前的所有历史数据，允许在一个visit内收集部分数据

    Args:
        visits: 所有就诊记录
        current_time: 疾病代码首次出现的时间
        index_set: 有效的代码集合
        days_before: 截止到疾病出现前几天（0表示直到疾病出现时刻）
    ...
    # 计算时间阈值
    hours_current = np.expm1(current_time) * (24 * 7)
    hours_threshold = hours_current - (days_before * 24)
    if hours_threshold < 0:
        hours_threshold = 0
    time_threshold = np.log1p(hours_threshold / (24 * 7))

    for visit_idx, visit in enumerate(visits):
        # 收集当前visit中时间早于current_time的codes
        current_visit_codes = []
        current_visit_times = []

        for code_idx, time in visit:
            # 只收集时间早于current_time的codes
            if time < current_time and code_idx in index_set and not pd.isna(time):
                current_visit_codes.append(code_idx)
                current_visit_times.append(time)

        # 如果有符合条件的codes，则添加到结果中
        if current_visit_codes:
            visit_codes.append(current_visit_codes)
            visit_times.append(current_visit_times)

        # 如果整个visit的最后一个时间点已经超过current_time，后面的visits不用再检查
        if visit[-1][1] >= current_time:
            break

    # 即使只有一个code也返回结果，避免过多过滤
    return [visit_codes, visit_times] if visit_codes else None
